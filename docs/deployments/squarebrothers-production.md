...
The script runs `npm ci` and `npm run build` locally, packages Next's standalone runtime and static assets, adds the prebuilt Linux x64 runtime for the pinned CPU-compatible `sharp` release, rsyncs that immutable release to the VPS, reloads PM2, verifies `/admin` locally, and keeps five releases. It does not install packages or build on the VPS.
